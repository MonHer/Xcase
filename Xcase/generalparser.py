...
def xmind_to_testsuites(xmind_content_dict):
    ...
    for sheet in xmind_content_dict:
        logger.debug('start to parse a sheet: %s' % sheet['title'])
        ...
        logger.debug('sheet(%s) parsing complete: %s' % (sheet['title'], suite.to_dict()))
        suites.append(suite)
...
def sheet_to_suite(root_topic):
    ...
    if separator in config['valid_sep']:
        logger.debug('find a valid separator for connecting testcase title: %s' % separator)
...
def parse_testsuite(suite_dict):
    ...
    logger.debug('start to parse a testsuite: %s' % testsuite.name)

    for cases_dict in suite_dict.get('topics', []):
        for case in recurse_parse_testcase(cases_dict):
            testsuite.testcase_list.append(case)

    logger.debug('testsuite(%s) parsing complete: %s' % (testsuite.name, testsuite.to_dict()))
...
def parse_a_testcase(case_dict, parent):
    ...
    # 用例名称
    testcase.name = gen_testcase_title([case_dict])
    # 用例前置条件 'note'
    ...
    logger.debug('finds a testcase: %s' % testcase.to_dict())
    return testcase
...
def parse_a_test_step(step_dict):
    ...
    logger.debug('finds a teststep: %s' % test_step.to_dict())
    return test_step
